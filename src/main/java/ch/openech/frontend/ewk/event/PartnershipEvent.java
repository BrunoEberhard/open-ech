...
public class PartnershipEvent extends PersonEventEditor<PartnershipEvent.Partnership> {
	...
	protected List<String> getXml(Person person, Partnership data, WriterEch0020 writerEch0020) throws Exception {
		...
		xmls.add(partnership(writerEch0020, data.dateOfMaritalStatus, data.partner1, data.partner2));
		if (Boolean.TRUE.equals(data.registerPartner2)) {
			xmls.add(partnership(writerEch0020, data.dateOfMaritalStatus, data.partner2, data.partner1));
		}
	...
	private String partnership(WriterEch0020 writerEch0020, LocalDate dateOfMaritalStatus, Person partner1, Person partner2) throws Exception {
		Relation relation = new Relation();
		relation.typeOfRelationship = TypeOfRelationship.Partner;
		relation.partner.setValue(partner2);
		return writerEch0020.partnership(partner1.personIdentification(), relation, dateOfMaritalStatus);
	}
